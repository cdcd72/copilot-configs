...
本儲存庫用於集中管理 GitHub Copilot 的自訂設定，包含 agents、instructions、prompts 與 hooks。

> 原由本儲存庫管理的 Agent Skills 已於 `8481b58` 遷移至 [skills](https://github.com/cdcd72/skills) 儲存庫，現已不再隨本專案維護。
...
- `.github/instructions/`：跨任務共用實作規範，目前僅以 `.keep` 保留目錄。
...
- 新增/調整 agent、hook、instruction、prompt 或 repo-level copilot 指示後，同步更新本 README 的「目前結構」與說明。

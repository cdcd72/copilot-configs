# Create a new .gitignore file for a project

1. Ask user to provide one of the following `.gitignore` template ids:
...
2. Then, you can use the following command to generate a `.gitignore` file based on the selected template:

   ```bash
   curl -sL https://www.toptal.com/developers/gitignore/api/{template-id} > .gitignore
   ```
